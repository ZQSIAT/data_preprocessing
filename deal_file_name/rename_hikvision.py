...
class RenameHikvision(object):
    ...
    @staticmethod
    def filter_sequence(key_words, src_list):
    ...
    def rename_hikvision(self, path):
        ...
        for i, ICount in enumerate(now_sequence_list):
            if ICount[0:2] == 'a0':
                ...
                times = self.filter_sequence(ICount[0:3], now_sequence_list).index(ICount) + 1
                pass
            else:
                sequence = 8
                times = self.filter_sequence(ICount[0:3], now_sequence_list).index(ICount) + 1
            ...
            for j, JCount in enumerate(os.listdir(now_subject_path + ICount)):
                ...
                dst_file = dst_root + self.new_name.format(option, people, camera, times, sequence)
                if not JCount[-4:] == '.mp4':
                    ...
                    pass
                if times > 3 and JCount[-4:] == '.mp4' and os.path.exists(src_file) and not os.path.exists(dst_file):
                    capture = cv2.VideoCapture(src_file)
                    if capture.isOpened():
                        os.rename(src_file, dst_file)
                        print("\"{:s}\" have done.".format(self.new_name.format(option, people, camera, times, sequence)))
                        logger.info("\"{:s}\" have done.".format(self.new_name.format(option, people, camera, times, sequence)))
                        pass
                    else:
                        print("Video \"{:s}\" was broken.".format(self.new_name.format(option, people, camera, times, sequence)))
                        logger.info("Video \"{:s}\"  was broken.".format(self.new_name.format(option, people, camera, times, sequence)))
                        pass
                    pass
                else:
...
if __name__ == "__main__":
    ...
    file_lst = my_rename_hikvision.generate_list()
    for subject_i in file_lst[0]:
        my_rename_hikvision.rename_hikvision(subject_i)
        pass
    # processing by pool map
    # my_pool = Pool(4)
    # my_task = my_pool.map(my_rename_hikvision.rename_hikvision, file_lst[0])
