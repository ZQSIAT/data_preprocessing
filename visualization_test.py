...
# It was designed for testing data transformation by zqs
def visualization_test(data_path="",start_frame=1,duration=8):

    data_list = os.listdir(data_path)
    data_list.sort()
    img_list = []
    for i in range(duration):
        img_b = cv2_loader(data_path + data_list[i + start_frame]).astype(np.float)
        img_c = cv2_loader(data_path + data_list[i + start_frame + 1]).astype(np.float)
        img_n = cv2_loader(data_path + data_list[i + start_frame + 2]).astype(np.float)
        [dy, dx] = np.gradient(img_c)
        dt = (img_n - img_b) / 2.0
        # d_norm = np.sqrt(np.square(dx) + np.square(dy) + np.square(dt) + 1e-8)
        # d_val = np.stack([dt / d_norm, dy / d_norm, dx / d_norm], axis=-1)
        # dx = (dx + 1) / 2
        dx = simple_scaling_0_1(dx)
        dy = simple_scaling_0_1(dy)
        dt = simple_scaling_0_1(dt)

        d_val = np.stack([dt, dy, dx], axis=-1)

        # d_val = simple_scaling_0_1(d_val)
        # d_val = preprocessing.normalize(d_val,'l1')
        fig = plt.figure()
        ...
        img_list.append(d_val)
        # print(img_b, "\t", img_c, "\t", img_n, "\n")
        # raise RuntimeError
        pass
    img_array = np.concatenate([np.expand_dims(x, 0) for x in img_list], axis=0)
    print(img_array.shape)
    # fig = plt.figure()

    dt_mean = np.mean(img_array[:, :, :, 0], axis=0)
    dy_mean = np.mean(img_array[:, :, :, 1], axis=0)
    dx_mean = np.mean(img_array[:, :, :, 2], axis=0)
    img_mean = np.stack([dt_mean, dy_mean, dx_mean], axis=-1)

    # ax1 = fig.add_subplot(111)
    # im = ax1.imshow(img_mean)
    # plt.colorbar(im)

    max_norm_index = np.argmax(np.square(img_array[:, :, :, 0]) + np.square(img_array[:, :, :, 1]) + np.square(img_array[:, :, :, 2]), axis=0)
    print(max_norm_index.shape)
    for i in range(max_norm_index.shape[0]):
        for j in range(max_norm_index.shape[1]):
            dt_mean = (img_array[max_norm_index[i, j], :, :, 0])
            # print(dx_mean.shape)
            dy_mean = (img_array[max_norm_index[i, j], :, :, 1])
            dx_mean = (img_array[max_norm_index[i, j], :, :, 2])
            # raise RuntimeError
            pass
        pass
    d_norm_max = np.sqrt(np.square(dx_mean) + np.square(dy_mean) + np.square(dt_mean) + 1e-8)
    img_norm = np.stack([dt_mean/d_norm_max, dy_mean/d_norm_max, dx_mean/d_norm_max], axis=-1)
    print(img_norm.shape)

    # ax2 = fig.add_subplot(111)
    # im = ax2.imshow(img_norm)
    # plt.colorbar(im)
    # plt.show()
    return img_norm, img_mean, d_val
    pass

def simple_scaling_0_1(array):
    max_array = np.max(array)
    min_array = np.min(array)
    ss_array = (array - min_array) / (max_array - min_array)
    return ss_array
    pass

if __name__ == "__main__":

    data_path = "F:/pycharm_preject/S017C001P008R001A052/"
    visualization_test(data_path, 25, 8)
    ...
    img_norm1, img_mean1, DOGV1 = visualization_test(data_path, start_frame1, duration1)
    img_norm2, img_mean2, DOGV2 = visualization_test(data_path, start_frame2, duration2)
    img_norm3, img_mean3, DOGV3 = visualization_test(data_path, start_frame3, duration3)

    fig = plt.figure()
    ax1 = fig.add_subplot(331)
    ax1.set_title('{:d}S {:d}F mean'.format(start_frame1, duration1), fontsize=8, color='r')
    im = ax1.imshow(img_mean1)
    # plt.colorbar(im)

    ax2 = fig.add_subplot(332)
    ax2.set_title('{:d}S {:d}F mean'.format(start_frame2, duration2), fontsize=8, color='r')
    im = ax2.imshow(img_mean2)
    # plt.colorbar(im)

    ax3 = fig.add_subplot(333)
    ax3.set_title('{:d}S {:d}F mean'.format(start_frame3, duration3), fontsize=8, color='r')
    im = ax3.imshow(img_mean3)
    # plt.colorbar(im)

    ax4 = fig.add_subplot(334)
    ax4.set_title('{:d}S {:d}F norm'.format(start_frame1, duration1), fontsize=8, color='g')
    im = ax4.imshow(img_norm1)
    # plt.colorbar(im)

    ax5 = fig.add_subplot(335)
    ax5.set_title('{:d}S {:d}F norm'.format(start_frame2, duration2), fontsize=8, color='g')
    im = ax5.imshow(img_norm2)
    # plt.colorbar(im)

    ax6 = fig.add_subplot(336)
    ax6.set_title('{:d}S {:d}F norm'.format(start_frame3, duration3), fontsize=8, color='g')
    im = ax6.imshow(img_norm3)

    ax7 = fig.add_subplot(337)
    ax7.set_title('{:d}S {:d}F DOGV'.format(start_frame1, duration1), fontsize=8, color='g')
    im = ax7.imshow(DOGV1)
    # plt.colorbar(im)

    ax8 = fig.add_subplot(338)
    ax8.set_title('{:d}S {:d}F DOGV'.format(start_frame2, duration2), fontsize=8, color='g')
    im = ax8.imshow(DOGV2)
    # plt.colorbar(im)

    ax9 = fig.add_subplot(339)
    ax9.set_title('{:d}S {:d}F DOGV'.format(start_frame3, duration3), fontsize=8, color='g')
    im = ax9.imshow(DOGV3)
